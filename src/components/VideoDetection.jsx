...
export default function VideoDetection({ setActiveTab, setSelectedVideo }) {
  ...
  const processVideoDirection = async () => {
    if (!selectedFile) {
      setError("Please select a video file first");
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${config.API_BASE_URL}/api/process-video-direction`, {
        method: "POST",
        body: formData,
      });
  ...
  const processVideoNoParking = async () => {
    if (!selectedFile) {
      setError("Please select a video file first");
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${config.API_BASE_URL}/api/process-video-noparking`, {
        method: "POST",
        body: formData,
      });
  ...
  useEffect(() => {
    ...
    frameViolations.forEach((v) => {
      ...
      ctx.fillText(
        `${v.type} (${(v.confidence * 100).toFixed(1)}%)`,
        x1,
        y1 - 5
      );
    });
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400">
            Upload and analyze traffic violation videos
          </p>
        </div>
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div className="bg-gray-700 rounded-lg p-4">
                      ...
                      <p className="text-2xl font-bold text-white">
                        {detectionStats.processedFrames}
                      </p>
                    </div>
                    <div className="bg-gray-700 rounded-lg p-4">
                      <h4 className="text-gray-400 text-sm font-medium">
                      ...
                      <p className="text-2xl font-bold text-red-400">
                        {detectionStats.violationsDetected}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div className="bg-gray-700 rounded-lg p-4">
                      ...
                      <p className="text-2xl font-bold text-white">
                        {detectionStats.processingTime}s
                      </p>
                    </div>
                    <div className="bg-gray-700 rounded-lg p-4">
                      <h4 className="text-gray-400 text-sm font-medium">
                    ...
                    results.violations.length > 0 && (
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-6">
                        {results.violations.map((violation, idx) => (
                          <div
                            key={idx}
                            className="bg-gray-900 rounded-lg shadow border border-gray-700 p-4 flex flex-col items-center"
                          >
                            <img
                            ...
                            <div className="w-full">
                              <p className="text-white font-semibold mb-1">
                                ...
                                {violation.type === "Number Plate" && (
                                  <span className="ml-2 inline-block px-2 py-0.5 text-xs bg-green-700 text-white rounded">
                                    Number Plate
                                  </span>
                                )}
                              </p>
